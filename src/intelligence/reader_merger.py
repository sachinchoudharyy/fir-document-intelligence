...
def merge_reader_data(final_json, reader_data):
    ...
    if reader_data.get("fir_number"):
        metadata["fir_number"] = reader_data["fir_number"]

    if reader_data.get("fir_datetime"):
        metadata["fir_datetime"] = reader_data["fir_datetime"]

    if reader_data.get("occurrence_datetime"):
        incident["occurrence_datetime"] = reader_data["occurrence_datetime"]

    # ---------- legal ----------

    if reader_data.get("sections"):
        legal["sections"] = list(set(reader_data["sections"]))

    if reader_data.get("acts"):
        legal["acts"] = list(set(reader_data["acts"]))

    # ---------- vehicle numbers (READER PRIORITY) ----------
    ...
    for v in vehicles:
        ...
        # if reader detected vehicle → ALWAYS use it
        if reader_vehicles and v.get("role") == "accused":
            v["vehicle_number"] = reader_vehicles[0]
        else:
            v["vehicle_number"] = None
